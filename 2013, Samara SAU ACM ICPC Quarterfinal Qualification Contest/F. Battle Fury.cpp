...
const int N = 4 * 200001, M = 998244353, OO = 0X3F3F3F3F3F3F3F3F;
...
auto Solve(int &n) {
    ...
    if (p == q) {
        int ans = 0 ;
        for (auto it : v) ans = max(ans, (it -1 ) /  p + 1);
        return ans ;
    }
    int l = 0, h = 1e9 +1 , ans = OO;
    while (l <= h) {
        ll mid = l + (h - l) / 2;
        if (rec(mid)) {
            ans = mid;
