...
const int N = 1e4 + 1, X = 101, EPS = 1e-12;
...
#define x real()
#define y imag()
T sq(pt point) { return point.x * point.x + point.y * point.y; }
T abs_(pt point) { return abs(point); }
...
pair<pt, double> circumCircle(pt a, pt b, pt c) {
    // make circle from 3 points return point (x,y),radius
    b = b - a, c = c - a;
    if (sgn(cross(b, c)) == 0) {
        return {{0, 0}, 0};
    }
    return {
        a + perp(b * sq(c) - c * sq(b)) / cross(b, c) / (T) (2),
...
void solve() {
    ...
    pt B2odam = {B / 2.0, 0};
    //(0,L) ,, (0,B/2)
    ld h = sqrtl(L * L - (B / 2.0) * (B / 2.0));
    pt foo2 = {0, h};
    ld a = abs(Bwraa - B2odam);
    ld b = abs(foo2 - B2odam);
    ld c = abs(foo2 - Bwraa);
    pt incenter = (a * foo2 + b * Bwraa + c * B2odam) / (a + b + c);
    pt circumcenter = circumCircle(foo2, Bwraa, B2odam).ff;
    cout << fixed << setprecision(15) << abs(incenter - circumcenter);
}
