...
auto Solve(const int &n) -> void {
    ...
    for (int i = 0; i < m; i++) {
        ...
        grid[u].emplace_back(v, prob);
        grid[v].emplace_back(u, prob);
        edge.emplace_back(u, v, prob);
    }

    auto dijkstra = [&](int start) -> vector<double> {
    ...
    auto st = dijkstra(0);
    auto ed = dijkstra(n - 1);

    double ans = 0.0;

    for (auto &[u, v, original_prob]: edge) {
        ans = max(ans, max(st[u] * 1.0 * ed[v], st[v] * 1.0 * ed[u]));
    }
    cout << fixed(20) << ans * 100.0 << endl;
}
