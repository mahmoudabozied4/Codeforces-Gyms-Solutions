...
const int N = 1e7 + 10 + 5, M = 1'000'000'007, OO = 0X3F3F3F3F3F3F3F3F;
...
auto Solve(const int &n) -> string {
    ...
    string ans;
    int red = a, green = b, blue = c;

    int rem = n - a - b - c;

    red += rem, green += rem, blue += rem;
    char prv = 'Z';
    for (int i = 0; i < n; i++) {
        if (red >= green && red >= blue && prv != 'R') {
            ans += 'R';
            red--;
        } else if (green >= blue && prv != 'G') {
            ans += 'G';
            green--;
        } else if (prv != 'B') {
            ans += 'B';
