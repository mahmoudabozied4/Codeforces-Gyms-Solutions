...
void solve() {
    ...
    vector<multiset<ll>> columns(m);
    for (int j = 0; j < m; ++j)
        for (int i = 0; i < n; ++i)
            columns[j].insert(v[i][j]);


    ll ans = 0;
    while (1) {
        if (columns[0].empty()) put(ans);
        ll prev = *columns[0].begin();
        columns[0].erase(columns[0].begin());

        for (int j = 1; j < m; ++j) {
            if (columns[j].empty()) put(ans);
            auto it = columns[j].lower_bound(prev);
            if (it == columns[j].end()) put(ans);
            prev = *it;
            columns[j].erase(it);
        }
        ans++;
